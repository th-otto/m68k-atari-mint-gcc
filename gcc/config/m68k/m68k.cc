...
/* Debug logging for RTX costs - set to 1 to enable */
#define M68K_COST_DEBUG 0

#if M68K_COST_DEBUG
/* Return mode suffix for assembly output */
static const char *
m68k_mode_suffix (machine_mode mode)
{
...
/* Return register name for assembly output */
static const char *
m68k_reg_name (unsigned int regno)
{
  ...
  static char pseudo_buf[16];
  if (regno < sizeof(names)/sizeof(names[0]))
    return names[regno];
  /* Pseudo register - show as pNN */
  snprintf (pseudo_buf, sizeof(pseudo_buf), "p%u", regno);
  return pseudo_buf;
}

/* Convert RTX to assembly-like string representation.
   Uses rotating static buffers to handle recursive calls. */
static const char *
m68k_rtx_to_asm (rtx x, machine_mode mode)
{
  /* Use 8 rotating buffers to handle nested expressions */
  #define NUM_ASM_BUFS 8
  #define ASM_BUF_SIZE 128
  static char bufs[NUM_ASM_BUFS][ASM_BUF_SIZE];
  static int buf_idx = 0;
  char *buf = bufs[buf_idx++ % NUM_ASM_BUFS];
  const char *suffix = m68k_mode_suffix (mode);

  if (x == NULL_RTX)
    {
      snprintf (buf, ASM_BUF_SIZE, "(null)");
      return buf;
    }
  ...
  switch (GET_CODE (x))
    {
    case REG:
      snprintf (buf, ASM_BUF_SIZE, "%%%s", m68k_reg_name (REGNO (x)));
      return buf;
	...
	if (val >= -128 && val <= 127)
	  snprintf (buf, ASM_BUF_SIZE, "#%ld", (long)val);
	else if (val >= -32768 && val <= 32767)
	  snprintf (buf, ASM_BUF_SIZE, "#%ld.w", (long)val);
	else
	  snprintf (buf, ASM_BUF_SIZE, "#%ld.l", (long)val);
	return buf;
      }

    case CONST_DOUBLE:
      snprintf (buf, ASM_BUF_SIZE, "#<double>%s", suffix);
      return buf;

    case CONST:
      snprintf (buf, ASM_BUF_SIZE, "#<const>%s", suffix);
      return buf;

    case SYMBOL_REF:
      snprintf (buf, ASM_BUF_SIZE, "%s%s", XSTR (x, 0), suffix);
      return buf;

    case LABEL_REF:
      snprintf (buf, ASM_BUF_SIZE, "<label>%s", suffix);
      return buf;
	...
	switch (GET_CODE (addr))
	  {
	  case REG:
	    snprintf (buf, ASM_BUF_SIZE, "(%%%s)%s",
		      m68k_reg_name (REGNO (addr)), suffix);
	    break;

	  case POST_INC:
	    snprintf (buf, ASM_BUF_SIZE, "(%%%s)+%s",
		      m68k_reg_name (REGNO (XEXP (addr, 0))), suffix);
	    break;
	  ...
	  case PLUS:
		...
		snprintf (buf, ASM_BUF_SIZE, "%ld(%%%s)%s",
			  (long)INTVAL (offset),
			  m68k_reg_name (REGNO (base)), suffix);
	      else if (REG_P (base) && REG_P (offset))
		snprintf (buf, ASM_BUF_SIZE, "(%%%s,%%%s)%s",
			  m68k_reg_name (REGNO (base)),
			  m68k_reg_name (REGNO (offset)), suffix);
	      else if (REG_P (base) && GET_CODE (offset) == MULT)
		{
		  rtx idx = XEXP (offset, 0);
		  rtx scale = XEXP (offset, 1);
		  if (REG_P (idx) && CONST_INT_P (scale))
		    snprintf (buf, ASM_BUF_SIZE, "(%%%s,%%%s*%ld)%s",
			      m68k_reg_name (REGNO (base)),
			      m68k_reg_name (REGNO (idx)),
			      (long)INTVAL (scale), suffix);
		  else
		    snprintf (buf, ASM_BUF_SIZE, "<mem-plus-mult>%s", suffix);
		}
	      else if (GET_CODE (base) == PLUS && CONST_INT_P (offset))
		{
		  /* d(An,Xn) form */
		  rtx b1 = XEXP (base, 0);
		  rtx b2 = XEXP (base, 1);
		  if (REG_P (b1) && REG_P (b2))
		    snprintf (buf, ASM_BUF_SIZE, "%ld(%%%s,%%%s)%s",
			      (long)INTVAL (offset),
			      m68k_reg_name (REGNO (b1)),
			      m68k_reg_name (REGNO (b2)), suffix);
		  else
		    snprintf (buf, ASM_BUF_SIZE, "<mem-indexed>%s", suffix);
		}
	      else if (REG_P (base) && SYMBOL_REF_P (offset))
		snprintf (buf, ASM_BUF_SIZE, "%s(%%%s)%s",
			  XSTR (offset, 0),
			  m68k_reg_name (REGNO (base)), suffix);
	      else
		snprintf (buf, ASM_BUF_SIZE, "<mem-plus>%s", suffix);
	    }
	    break;

	  case CONST_INT:
	    snprintf (buf, ASM_BUF_SIZE, "$%lx%s",
		      (unsigned long)INTVAL (addr), suffix);
	    break;

	  case SYMBOL_REF:
	    snprintf (buf, ASM_BUF_SIZE, "%s%s", XSTR (addr, 0), suffix);
	    break;

	  default:
	    snprintf (buf, ASM_BUF_SIZE, "<mem:%s>%s",
		      GET_RTX_NAME (GET_CODE (addr)), suffix);
	  }
	return buf;
      }

    case PLUS:
      snprintf (buf, ASM_BUF_SIZE, "(add%s %s,%s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case MINUS:
      snprintf (buf, ASM_BUF_SIZE, "(sub%s %s,%s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case MULT:
      snprintf (buf, ASM_BUF_SIZE, "(mul%s %s,%s)",
		GET_MODE_SIZE (mode) <= 2 ? "s.w" : ".l",
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case DIV:
      snprintf (buf, ASM_BUF_SIZE, "(divs%s %s,%s)",
		GET_MODE_SIZE (mode) <= 2 ? ".w" : ".l",
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case UDIV:
      snprintf (buf, ASM_BUF_SIZE, "(divu%s %s,%s)",
		GET_MODE_SIZE (mode) <= 2 ? ".w" : ".l",
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;
    ...
    case MOD:
      snprintf (buf, ASM_BUF_SIZE, "(divs%s %s,%s)[rem]",
		GET_MODE_SIZE (mode) <= 2 ? ".w" : ".l",
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;
    ...
    case UMOD:
      snprintf (buf, ASM_BUF_SIZE, "(divu%s %s,%s)[rem]",
		GET_MODE_SIZE (mode) <= 2 ? ".w" : ".l",
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;
    ...
    case AND:
      snprintf (buf, ASM_BUF_SIZE, "(and%s %s,%s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case IOR:
      snprintf (buf, ASM_BUF_SIZE, "(or%s %s,%s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case XOR:
      snprintf (buf, ASM_BUF_SIZE, "(eor%s %s,%s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case NOT:
      snprintf (buf, ASM_BUF_SIZE, "(not%s %s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case NEG:
      snprintf (buf, ASM_BUF_SIZE, "(neg%s %s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case ASHIFT:
      snprintf (buf, ASM_BUF_SIZE, "(lsl%s %s,%s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case ASHIFTRT:
      snprintf (buf, ASM_BUF_SIZE, "(asr%s %s,%s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case LSHIFTRT:
      snprintf (buf, ASM_BUF_SIZE, "(lsr%s %s,%s)", suffix,
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case ZERO_EXTEND:
      snprintf (buf, ASM_BUF_SIZE, "zext%s(%s)",
		suffix, m68k_rtx_to_asm (XEXP (x, 0), GET_MODE (XEXP (x, 0))));
      return buf;

    case SIGN_EXTEND:
      snprintf (buf, ASM_BUF_SIZE, "sext%s(%s)",
		suffix, m68k_rtx_to_asm (XEXP (x, 0), GET_MODE (XEXP (x, 0))));
      return buf;

    case TRUNCATE:
      snprintf (buf, ASM_BUF_SIZE, "trunc%s(%s)",
		suffix, m68k_rtx_to_asm (XEXP (x, 0), GET_MODE (XEXP (x, 0))));
      return buf;

    case COMPARE:
      snprintf (buf, ASM_BUF_SIZE, "cmp%s %s,%s", suffix,
		m68k_rtx_to_asm (XEXP (x, 0), mode),
		m68k_rtx_to_asm (XEXP (x, 1), mode));
      return buf;
	...
	rtx dst = XEXP (x, 0);
	rtx src = XEXP (x, 1);
	machine_mode set_mode = GET_MODE (dst);
	if (set_mode == VOIDmode)
	  set_mode = GET_MODE (src);
	snprintf (buf, ASM_BUF_SIZE, "move%s %s,%s",
		  m68k_mode_suffix (set_mode),
		  m68k_rtx_to_asm (src, set_mode),
		  m68k_rtx_to_asm (dst, set_mode));
	return buf;
      }

    case CALL:
      snprintf (buf, ASM_BUF_SIZE, "jsr %s",
		m68k_rtx_to_asm (XEXP (x, 0), Pmode));
      return buf;

    case IF_THEN_ELSE:
      snprintf (buf, ASM_BUF_SIZE, "<if_then_else>");
      return buf;

    case SUBREG:
      snprintf (buf, ASM_BUF_SIZE, "subreg%s(%s)",
		suffix, m68k_rtx_to_asm (SUBREG_REG (x), GET_MODE (SUBREG_REG (x))));
      return buf;

    case STRICT_LOW_PART:
      snprintf (buf, ASM_BUF_SIZE, "strict_low(%s)",
		m68k_rtx_to_asm (XEXP (x, 0), mode));
      return buf;

    case ZERO_EXTRACT:
      snprintf (buf, ASM_BUF_SIZE, "bfextu(%s,%s,%s)",
		m68k_rtx_to_asm (XEXP (x, 0), mode),
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 2), mode));
      return buf;

    case SIGN_EXTRACT:
      snprintf (buf, ASM_BUF_SIZE, "bfexts(%s,%s,%s)",
		m68k_rtx_to_asm (XEXP (x, 0), mode),
		m68k_rtx_to_asm (XEXP (x, 1), mode),
		m68k_rtx_to_asm (XEXP (x, 2), mode));
      return buf;
    ...
    case PRE_DEC:
      snprintf (buf, ASM_BUF_SIZE, "-(%%%s)",
		m68k_reg_name (REGNO (XEXP (x, 0))));
      return buf;
    ...
    /* Comparison codes - shown as Scc conditions */
    case EQ:
      snprintf (buf, ASM_BUF_SIZE, "seq");
      return buf;
    case NE:
      snprintf (buf, ASM_BUF_SIZE, "sne");
      return buf;
    case GT:
      snprintf (buf, ASM_BUF_SIZE, "sgt");
      return buf;
    case GE:
      snprintf (buf, ASM_BUF_SIZE, "sge");
      return buf;
    case LT:
      snprintf (buf, ASM_BUF_SIZE, "slt");
      return buf;
    case LE:
      snprintf (buf, ASM_BUF_SIZE, "sle");
      return buf;
    case GTU:
      snprintf (buf, ASM_BUF_SIZE, "shi");
      return buf;
    case GEU:
      snprintf (buf, ASM_BUF_SIZE, "scc");
      return buf;
    case LTU:
      snprintf (buf, ASM_BUF_SIZE, "scs");
      return buf;
    case LEU:
      snprintf (buf, ASM_BUF_SIZE, "sls");
      return buf;

    default:
      snprintf (buf, ASM_BUF_SIZE, "<%s>%s", GET_RTX_NAME (GET_CODE (x)), suffix);
      return buf;
    }
}

/* Log RTX cost calculation */
static void
m68k_log_rtx_cost (rtx x, machine_mode mode, int outer_code,
		   int opno, int cost, bool speed)
{
  const char *outer_name = outer_code >= 0 ? GET_RTX_NAME ((enum rtx_code)outer_code) : "?";
  const char *opno_name = opno == 0 ? "dst" : "src";
  fprintf (stderr, "COST: %3d  %s  (outer=%s %s %s)\n",
	   cost,
	   m68k_rtx_to_asm (x, mode),
	   outer_name, opno_name,
	   speed ? "speed" : "size");
}

/* Log address cost calculation */
static void
m68k_log_address_cost (rtx x, machine_mode mode, int cost, bool speed)
{
  fprintf (stderr, "ADDR: %3d  %s  (dst %s)\n",
	   cost,
	   m68k_rtx_to_asm (x, mode),
	   speed ? "speed" : "size");
}
#endif /* M68K_COST_DEBUG */
...
{
  bool result = m68k_rtx_costs_impl (x, mode, outer_code, opno, total, speed);
#if M68K_COST_DEBUG
  if (result)
    m68k_log_rtx_cost (x, mode, outer_code, opno, *total, speed);
#endif
  return result;
}
...
#if M68K_COST_DEBUG
  m68k_log_address_cost (&mem, mode, total, speed);
#endif
  return total;
}
