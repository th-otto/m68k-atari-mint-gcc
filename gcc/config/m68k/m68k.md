...
;; Processor type.
(define_attr "cpu" "cfv1, cfv2, cfv3, cfv4, m68060, unknown"
...
;; Code iterators for read-modify-write with post-increment.
(define_code_iterator rmw_op [plus minus and ior xor])
(define_code_iterator rmw_comm_op [plus and ior xor])
(define_code_attr rmw_insn [(plus "add") (minus "sub") (and "and") (ior "or") (xor "eor")])
(define_code_attr rmw_flags [(plus "noov") (minus "noov") (and "yes") (ior "yes") (xor "yes")])
...
(define_insn "beq0_di"
  [(set (pc)
    (if_then_else (eq (match_operand:DI 0 "general_operand" "d,a,o,<>")
        ...
        (pc)))
   (clobber (match_scratch:SI 2 "=d,X,&d,d"))]
  ""
{
...
(define_mode_attr scc0_constraints [(QI "=d,d,d") (HI "=d,d,d,d,d,d") (SI "=d,d,d,d,d,d")])
(define_mode_attr cmp1_constraints [(QI "dn,dm,>") (HI "d,rnm,d,n,m,>") (SI "r,r,r,mr,ma,>")])
(define_mode_attr cmp2_constraints [(QI "dm,nd,>") (HI "n,d,rnm,m,n,>") (SI "mrC0,mr,ma,KTrC0,Ksr,>")])
...
;; On 68000/68010, testing an address register against zero requires
;; cmp.w #0,%aN (4 bytes, 12 cycles) because tst.l doesn't work on
;; address registers.  These patterns use a scratch data register to
;; emit move.l %aN,%dN (2 bytes, 4 cycles) which sets the condition
;; codes identically.  The subsequent tst.l is elided by CC tracking
;; in m68k_find_flags_value (output_move_simode sets flags_operand1).
;; Matched via peephole2 which adds the clobber when a scratch data
;; register is available, forming a single insn that cprop_hardreg
;; cannot decompose.
(define_insn "*cbranchsi4_areg_zero"
  [(set (pc)
	(if_then_else (match_operator 0 "ordered_comparison_operator"
...
{
  rtx_code code = GET_CODE (operands[0]);
  /* Check if CC is already valid for the address register.  */
  rtx_code elided = m68k_find_flags_value (operands[1], const0_rtx, code);
  if (elided != UNKNOWN)
    return m68k_output_branch_integer (elided);
  /* CC not valid — emit move.l aN,dN to set CC.  */
  rtx move_ops[2] = { operands[2], operands[1] };
  output_asm_insn (output_move_simode (move_ops), move_ops);
  code = m68k_output_compare_si (operands[2], const0_rtx, code);
  return m68k_output_branch_integer (code);
}
  [(set_attr "flags_valid" "set")])

(define_insn "*cbranchsi4_areg_zero_rev"
  [(set (pc)
	(if_then_else (match_operator 0 "ordered_comparison_operator"
		       [(match_operand:SI 1 "register_operand" "a")
			(const_int 0)])
  ...
  "!TARGET_68020 && !TARGET_COLDFIRE"
{
  ...
  rtx_code elided = m68k_find_flags_value (operands[1], const0_rtx, code);
  if (elided != UNKNOWN)
    return m68k_output_branch_integer_rev (elided);
  /* CC not valid — emit move.l aN,dN to set CC.  */
  rtx move_ops[2] = { operands[2], operands[1] };
  output_asm_insn (output_move_simode (move_ops), move_ops);
  code = m68k_output_compare_si (operands[2], const0_rtx, code);
...
;; Combined mem-to-mem move + conditional branch.
;; The move sets CC (N, Z based on value moved, V=0, C=0), so the
;; branch can test the condition directly without an explicit tst/cmp.
;; Emitted by the peephole2 that eliminates a register intermediate
;; from load+store+branch sequences (e.g. strcpy inner loops).
;; Not valid on ColdFire which lacks mem-to-mem moves.
(define_insn "*mov<I:mode>_mem_cbranch"
  [(set (pc)
	(if_then_else (match_operator 0 "ordered_comparison_operator"
		       [(match_operand:I 1 "register_operand" "d")
			(const_int 0)])
		      (label_ref (match_operand 2 ""))
		      (pc)))
   (set (match_operand:I 3 "memory_operand" "=m")
	(match_operand:I 4 "memory_operand" "m"))
   (clobber (match_dup 1))]
  "reload_completed && !TARGET_COLDFIRE"
{
  output_asm_insn ("move<sz> %4,%3", operands);
  /* m68k_output_branch_integer uses %l3 for the label, but our label
     is operand 2.  Swap so %l3 resolves to the label.  */
  std::swap (operands[2], operands[3]);
  return m68k_output_branch_integer (GET_CODE (operands[0]));
}
  [(set_attr "flags_valid" "no")])

(define_insn "*mov<I:mode>_mem_cbranch_rev"
  [(set (pc)
	(if_then_else (match_operator 0 "ordered_comparison_operator"
		       [(match_operand:I 1 "register_operand" "d")
			(const_int 0)])
  ...
  "reload_completed && !TARGET_COLDFIRE"
{
  output_asm_insn ("move<sz> %4,%3", operands);
  std::swap (operands[2], operands[3]);
  return m68k_output_branch_integer_rev (GET_CODE (operands[0]));
}
  [(set_attr "flags_valid" "no")])
...
;; btst with HImode shift-then-test-bit-0 pattern.
;; In -mshort mode, combine produces (zero_extract:SI (subreg:SI
;;   (lshiftrt:HI data shift_amt) 0) 1 31) instead of the canonical
;; (zero_extract:SI reg 1 (minus:SI 31 pos)).  The shift amount is the
;; hardware bit number directly: shifting right by N then testing bit 0
;; equals testing bit N of the original.
(define_insn "*cbranchsi4_btst_shifted_hi"
  [(set (pc)
	(if_then_else (match_operator 0 "equality_comparison_operator"
		       [(zero_extract:SI
			  (subreg:SI (lshiftrt:HI
			    (match_operand:HI 1 "register_operand" "d")
			    (match_operand:HI 2 "register_operand" "d")) 0)
			  (const_int 1)
			  (const_int 31))
			(const_int 0)])
		      (label_ref (match_operand 3 ""))
...
{
  rtx_code code = GET_CODE (operands[0]);
  code = m68k_output_btst (operands[2], operands[1], code, 15);
  return m68k_output_branch_integer (code);
})
...
;; Single-bit extraction via btst+sne.
;; Variant 1: minus encoding (used by peephole2 output).
(define_insn "cstore_btst<mode>_insn"
  [(set (match_operand:QI 0 "register_operand" "=d")
	(match_operator:QI 1 "equality_comparison_operator"
	 [(zero_extract:SI (match_operand:BTST 2 "<btst_predicate>" "<btst_constraint>")
			   (const_int 1)
			   (minus:SI (const_int <btst_range>)
				     (match_operand:SI 3 "general_operand" "di")))
	  (const_int 0)]))]
  "!(TARGET_68020 && TARGET_BITFIELD)
   && (!REG_P (operands[2]) || !CONST_INT_P (operands[3])
       || IN_RANGE (INTVAL (operands[3]), 0, 31))"
{
  rtx_code code = GET_CODE (operands[1]);
  code = m68k_output_btst (operands[3], operands[2], code, <btst_range>);
  return m68k_output_scc (code);
}
  [(set_attr "type" "scc")])

;; Variant 2: raw const_int encoding (used by combine).
;; Handles both memory and register sources with address adjustment.
(define_insn "cstore_btst_insn_1"
  [(set (match_operand:QI 0 "register_operand" "=d")
	(match_operator:QI 1 "equality_comparison_operator"
	 [(zero_extract:SI (match_operand:SI 2 "nonimmediate_operand" "do")
			   (const_int 1)
			   (match_operand:SI 3 "const_int_operand" "n"))
	  (const_int 0)]))]
  "!(TARGET_68020 && TARGET_BITFIELD)
   && (!REG_P (operands[2]) || IN_RANGE (INTVAL (operands[3]), 0, 31))"
{
  rtx_code code = GET_CODE (operands[1]);
  if (GET_CODE (operands[2]) == MEM)
    {
      operands[2] = adjust_address (operands[2], QImode,
				    INTVAL (operands[3]) / 8);
      operands[3] = GEN_INT (7 - INTVAL (operands[3]) % 8);
      code = m68k_output_btst (operands[3], operands[2], code, 7);
    }
  else
    {
      operands[3] = GEN_INT (31 - INTVAL (operands[3]));
      code = m68k_output_btst (operands[3], operands[2], code, 31);
    }
  return m68k_output_scc (code);
})
...
(define_insn ""
  [(set (match_operand:QI 0 "nonimmediate_operand" "=d,d,*a,*a,m")
...
(define_insn ""
  [(set (match_operand:QI 0 "nonimmediate_operand" "=d<Q>,dm,U,d,*a,*a")
	(match_operand:QI 1 "general_src_operand" "dmi,d<Q>,U,a,di,a"))]
  "TARGET_COLDFIRE"
  "* return output_move_qimode (operands);"
  [(set_attr "flags_valid" "set")])
...
;; Read-modify-write with post-increment: OP.x Dn,(An)+
;; These must be defined before the general *addsi3_internal etc. patterns
;; so that recog matches the more specific post_inc form first.

;; Standard operand order: (op (mem:I (An)) Dn) -> (An)+
(define_insn "*<rmw_insn><I:mode>_rmw_postinc"
  [(set (mem:I (post_inc:SI (match_operand:SI 0 "register_operand" "+a")))
	(rmw_op:I (mem:I (match_dup 0))
		  (match_operand:I 1 "register_operand" "d")))]
  "!TARGET_COLDFIRE"
{ return MOTOROLA ? "<rmw_insn><sz> %1,(%0)+" : "<rmw_insn><sz> %1,%0@+"; }
  [(set_attr "flags_valid" "<rmw_flags>")])

;; Reversed commutative: (op Dn (mem:I (An))) -> (An)+
(define_insn "*<rmw_insn><I:mode>_rmw_postinc_rev"
  [(set (mem:I (post_inc:SI (match_operand:SI 0 "register_operand" "+a")))
	(rmw_comm_op:I (match_operand:I 1 "register_operand" "d")
		       (mem:I (match_dup 0))))]
  "!TARGET_COLDFIRE"
{ return MOTOROLA ? "<rmw_insn><sz> %1,(%0)+" : "<rmw_insn><sz> %1,%0@+"; }
  [(set_attr "flags_valid" "<rmw_flags>")])
...
(define_insn "*addsi3_internal"
  [(set (match_operand:SI 0 "nonimmediate_operand" "=m,?a,?a,d,a")
        (plus:SI (match_operand:SI 1 "general_operand" "%0,a,rJK,0,0")
                 (match_operand:SI 2 "general_src_operand" "dIKLTCp,rJK,a,mSrIKLTCp,mSrIKLsCp")))]


  "! TARGET_COLDFIRE"
  "* return output_addsi3 (operands);"
  [(set_attr "type" "alu_l,alu_l,lea,alu_l,alu_l")
...
;; The muls instruction requires both the source and destination in a data
;; register (tied via %0).  Use register_operand for operand 1 to make
;; this explicit to combine and fwprop — otherwise they can merge a memory
;; load into the multiply, creating (mult (mem) (mem)) which forces LRA to
;; create a reload pseudo and may spill a register unnecessarily.
(define_insn "mulhi3"
  [(set (match_operand:HI 0 "nonimmediate_operand" "=d")
...
;; Same as above but with sign_extend around the constant.
;; This pattern can be generated by widening_mul or combine when
;; a narrow multiply is cast to a wider type.
(define_insn "*mulhisisi3_s_sext"
  [(set (match_operand:SI 0 "nonimmediate_operand" "=d")
	(mult:SI (sign_extend:SI
		  (match_operand:HI 1 "nonimmediate_operand" "%0"))
		 (sign_extend:SI
...
(define_insn "andsi3_internal"
  [(set (match_operand:SI 0 "not_sp_operand" "=m,d")
	(and:SI (match_operand:SI 1 "general_operand" "%0,0")
		(match_operand:SI 2 "general_src_operand" "dKTCp,dmSMCp")))]
  "!TARGET_COLDFIRE"
{
  return output_andsi3 (operands);
}
...
(define_insn "iorsi3_internal"
  [(set (match_operand:SI 0 "nonimmediate_operand" "=m,d")
	(ior:SI (match_operand:SI 1 "general_operand" "%0,0")
                (match_operand:SI 2 "general_src_operand" "dKTCp,dmSMTCp")))]
  "! TARGET_COLDFIRE"
{
...
;; Pack two HImode values into SImode: (hi1 << 16) | hi2
;; Generated by combiner when constructing small structs from two 16-bit values.
;; Emits: move.w %1,%0; swap %0; move.w %2,%0
;; Uses match_operand with const_int_operand predicate to ensure value-based
;; matching instead of pointer comparison in the generated recognition code.
(define_insn "*pack_hi_hi_to_si"
  [(set (match_operand:SI 0 "register_operand" "=&d,&d")
        (ior:SI
          (ashift:SI (zero_extend:SI (match_operand:HI 1 "register_operand" "da,0"))
                     (match_operand:SI 3 "const_int_operand" "n,n"))
          (zero_extend:SI (match_operand:HI 2 "register_operand" "da,da"))))]
  "INTVAL (operands[3]) == 16"
  "move%.w %1,%0\;swap %0\;move%.w %2,%0")

;; Same pattern with operands swapped (IOR is commutative).
(define_insn "*pack_hi_hi_to_si_swapped"
  [(set (match_operand:SI 0 "register_operand" "=&d,&d")
        (ior:SI
          (zero_extend:SI (match_operand:HI 2 "register_operand" "da,da"))
          (ashift:SI (zero_extend:SI (match_operand:HI 1 "register_operand" "da,0"))
...
;; Insert HImode value into high 16 bits of SImode register, preserving low bits.
;; The combiner generates this zero_extract form when it recognizes:
;;   (ior (and reg 0xffff) (ashift (zero_extend hi) 16))
;; is equivalent to inserting hi into the high 16 bits.
;; Uses swap-move-swap: 3 instructions instead of and+or (5 instructions).
(define_insn "*insv_hi_to_si_high16"
  [(set (zero_extract:SI (match_operand:SI 0 "register_operand" "+d")
                         (const_int 16)
                         (const_int 0))
        (zero_extend:SI (match_operand:HI 1 "register_operand" "da")))]
  ""
  "swap %0\;move%.w %1,%0\;swap %0")

;; Alternative IOR form for inserting HImode into high 16 bits.
;; Matches: dst = (dst & 0xffff) | (src << 16)
;; The combiner generates this when it doesn't simplify to zero_extract.
(define_insn "*iorsi_insert_hi_high16"
  [(set (match_operand:SI 0 "register_operand" "+d")
        (ior:SI (and:SI (match_dup 0)
                        (const_int 65535))
                (ashift:SI (zero_extend:SI (match_operand:HI 1 "register_operand" "da"))
                           (match_operand:SI 2 "const_int_operand" "n"))))]
  "INTVAL (operands[2]) == 16"
  "swap %0\;move%.w %1,%0\;swap %0")

;; Same with operands swapped (IOR is commutative).
(define_insn "*iorsi_insert_hi_high16_swapped"
  [(set (match_operand:SI 0 "register_operand" "+d")
        (ior:SI (ashift:SI (zero_extend:SI (match_operand:HI 1 "register_operand" "da"))
                           (match_operand:SI 2 "const_int_operand" "n"))
                (and:SI (match_dup 0)
                        (const_int 65535))))]
  "INTVAL (operands[2]) == 16"
...
;; Version with extra AND mask on the shifted value (combiner generates this).
;; Matches: dst = (dst & 0xffff) | ((src << 16) & 0xffff0000)
(define_insn "*iorsi_insert_hi_high16_masked"
  [(set (match_operand:SI 0 "register_operand" "+d")
        (ior:SI (and:SI (match_dup 0)
                        (const_int 65535))
                (and:SI (ashift:SI (zero_extend:SI (match_operand:HI 1 "register_operand" "da"))
                                   (match_operand:SI 2 "const_int_operand" "n"))
                        (const_int -65536))))]
  "INTVAL (operands[2]) == 16"
  "swap %0\;move%.w %1,%0\;swap %0")

;; Same with IOR operands swapped.
(define_insn "*iorsi_insert_hi_high16_masked_swapped"
  [(set (match_operand:SI 0 "register_operand" "+d")
        (ior:SI (and:SI (ashift:SI (zero_extend:SI (match_operand:HI 1 "register_operand" "da"))
                                   (match_operand:SI 2 "const_int_operand" "n"))
                        (const_int -65536))
                (and:SI (match_dup 0)
                        (const_int 65535))))]
...
;; Peephole2: Combine high-16 insert followed by low-16 set into 3 instructions.
(define_peephole2
  ...
  ""
  [(set (match_dup 0)
        (ior:SI (ashift:SI (zero_extend:SI (match_dup 1))
                           (const_int 16))
                (zero_extend:SI (match_dup 2))))])

;; Same pattern but with HImode register in strict_low_part instead of subreg
(define_peephole2
  [(set (zero_extract:SI (match_operand:SI 0 "register_operand" "")
                         (const_int 16)
                         (const_int 0))
        (zero_extend:SI (match_operand:HI 1 "register_operand" "")))
   (set (strict_low_part (match_operand:HI 3 "register_operand" ""))
        (match_operand:HI 2 "register_operand" ""))]
...
;; Peephole2: Optimize struct packing from 5 instructions to 3
(define_peephole2
  [(set (match_operand:SI 0 "register_operand" "")
  ...
  "INTVAL (operands[5]) == 16
   && INTVAL (operands[6]) == 0xffff
   && REG_P (operands[2])
   && (REG_P (operands[3])
       ? REGNO (operands[2]) == REGNO (operands[3])
       : SUBREG_P (operands[3]) && REGNO (operands[2]) == REGNO (SUBREG_REG (operands[3])))
   && peep2_reg_dead_p (5, operands[0])"
  [(set (match_dup 2)
        (ior:SI (ashift:SI (zero_extend:SI (match_dup 7))
                           (const_int 16))
                (zero_extend:SI (match_dup 4))))]
  {
    operands[7] = gen_lowpart (HImode, operands[1]);
  })

;; Note: High-word insertion optimization (ashift + and + ior -> swap + move.w + swap)
;; is handled by the m68k_highword_opt pass which runs after peephole2 and has access
;; to more accurate register liveness information.
...
(define_insn "ashldi_sexthi"
  [(set (match_operand:DI 0 "nonimmediate_operand" "=m,a,d")
    (ashift:DI (sign_extend:DI (match_operand:HI 1 "general_operand" "rm,rm,rm"))
        (const_int 32)))
    (clobber (match_scratch:SI 2 "=a,X,X"))]
  ""
{
  if (GET_CODE (operands[0]) == MEM)
    {
...
;; Combine andi.l #$ffff + clr.w into moveq #0.
;; With -mshort, struct zeroing (e.g. point_s{0,0}) generates:
;;   and.l #65535,%d0  (clear high word)
;;   clr.w %d0         (clear low word)
;; Together these zero all 32 bits — replace with a single moveq #0.
...
;; "add.x %0,%0" is the same size as "lsl.x #1,%0" and at least as
;; fast on every m68k variant.  On 68000/68010 it is significantly
;; faster (4/4/8 vs 8/8/10 cycles for b/w/l).
...
(define_peephole2
  [(set (match_operand:QI 0 "register_operand" "")
	(ashift:QI (match_operand:QI 1 "register_operand" "")
		   (const_int 1)))]
  "REGNO (operands[0]) == REGNO (operands[1])"
  [(set (match_dup 0) (plus:QI (match_dup 0) (match_dup 0)))])
...
(define_peephole2
  [(set (match_operand:HI 0 "register_operand" "")
	(ashift:HI (match_operand:HI 1 "register_operand" "")
		   (const_int 1)))]
  "REGNO (operands[0]) == REGNO (operands[1])"
  [(set (match_dup 0) (plus:HI (match_dup 0) (match_dup 0)))])
...
(define_peephole2
  [(set (match_operand:SI 0 "register_operand" "")
	(ashift:SI (match_operand:SI 1 "register_operand" "")
		   (const_int 1)))]
  "REGNO (operands[0]) == REGNO (operands[1])"
  [(set (match_dup 0) (plus:SI (match_dup 0) (match_dup 0)))])

;; Single-bit extraction via btst+sne.
;; On 68000/68010, shifts cost 2 extra cycles per shift count, making
;; the btst+sne sequence faster for single-bit extraction.
;;
;; Signed extraction: GCC generates ashift:QI by K + ashiftrt:QI by 7
;; to extract a signed 1-bit field.  ashift moves the target bit to
;; the MSB, ashiftrt sign-extends.  btst+sne produces 0x00/0xFF which
;; is the same result.  Always profitable: saves 20+2K cycles (memory)
;; or 14+2K cycles (register) vs 14 or 12 for btst+sne.

;; Pattern B: QI memory signed (3->1 RTL insn: load+ashift+ashiftrt -> btst+sne)
(define_peephole2
  [(set (match_operand:QI 0 "register_operand")
	(match_operand:QI 1 "memory_operand"))
   (set (match_dup 0)
	(ashift:QI (match_dup 0) (match_operand:SI 2 "const_int_operand")))
   (set (match_dup 0)
	(ashiftrt:QI (match_dup 0) (const_int 7)))]
  "flag_m68k_btst_extract && TUNE_68000_10
   && IN_RANGE (INTVAL (operands[2]), 1, 7)"
  [(set (match_dup 0)
	(ne:QI (zero_extract:SI (match_dup 1) (const_int 1)
				(minus:SI (const_int 7) (match_dup 3)))
	       (const_int 0)))]
  "operands[3] = GEN_INT (7 - INTVAL (operands[2]));")

;; Pattern A: QI memory unsigned (3->2 RTL insns: load+lsr+and -> btst+sne+neg)
(define_peephole2
  [(set (match_operand:QI 0 "register_operand")
	(match_operand:QI 1 "memory_operand"))
   (set (match_dup 0)
	(lshiftrt:QI (match_dup 0) (match_operand:SI 2 "const_int_operand")))
   (set (match_dup 0)
	(and:QI (match_dup 0) (const_int 1)))]
  "flag_m68k_btst_extract && TUNE_68000_10
   && IN_RANGE (INTVAL (operands[2]), 1, 7)"
  [(set (match_dup 0)
	(ne:QI (zero_extract:SI (match_dup 1) (const_int 1)
				(minus:SI (const_int 7) (match_dup 2)))
	       (const_int 0)))
   (set (match_dup 0) (neg:QI (match_dup 0)))])

;; Pattern D: QI register signed (2->1 RTL insn: ashift+ashiftrt -> btst+sne)
(define_peephole2
  [(set (match_operand:QI 0 "register_operand")
	(ashift:QI (match_dup 0) (match_operand:SI 2 "const_int_operand")))
   (set (match_dup 0)
	(ashiftrt:QI (match_dup 0) (const_int 7)))]
  "flag_m68k_btst_extract && TUNE_68000_10 && optimize_insn_for_speed_p ()
   && IN_RANGE (INTVAL (operands[2]), 1, 7)"
  [(set (match_dup 0)
	(ne:QI (zero_extract:SI (match_dup 3) (const_int 1)
				(minus:SI (const_int 31) (match_dup 4)))
	       (const_int 0)))]
  "operands[3] = gen_rtx_REG (SImode, REGNO (operands[0]));
   operands[4] = GEN_INT (7 - INTVAL (operands[2]));")

;; Pattern C: QI register unsigned (2->2 RTL insns: lsr+and -> btst+sne+neg)
(define_peephole2
  [(set (match_operand:QI 0 "register_operand")
	(lshiftrt:QI (match_dup 0) (match_operand:SI 2 "const_int_operand")))
   (set (match_dup 0)
	(and:QI (match_dup 0) (const_int 1)))]
  "flag_m68k_btst_extract && TUNE_68000_10 && optimize_insn_for_speed_p ()
   && IN_RANGE (INTVAL (operands[2]), 4, 7)"
  [(set (match_dup 0)
	(ne:QI (zero_extract:SI (match_dup 3) (const_int 1)
				(minus:SI (const_int 31) (match_dup 2)))
	       (const_int 0)))
   (set (match_dup 0) (neg:QI (match_dup 0)))]
  "operands[3] = gen_rtx_REG (SImode, REGNO (operands[0]));")

;; Pattern E: HI register unsigned with const_int shift (2->2 RTL insns)
;; (val >> N) & 1 where val is HImode, N in 4..15.
;; Shifts 4-8 use const_int operand directly; shifts 9-15 use Pattern F.
(define_peephole2
  [(set (match_operand:HI 0 "register_operand")
	(lshiftrt:HI (match_dup 0) (match_operand:SI 2 "const_int_operand")))
   (set (match_dup 0)
	(and:HI (match_dup 0) (const_int 1)))]
  "flag_m68k_btst_extract && TUNE_68000_10 && optimize_insn_for_speed_p ()
   && IN_RANGE (INTVAL (operands[2]), 4, 15)"
  [(set (match_dup 3)
	(ne:QI (zero_extract:SI (match_dup 4) (const_int 1)
				(minus:SI (const_int 31) (match_dup 2)))
	       (const_int 0)))
   (set (match_dup 3) (neg:QI (match_dup 3)))]
  "operands[3] = gen_rtx_REG (QImode, REGNO (operands[0]));
   operands[4] = gen_rtx_REG (SImode, REGNO (operands[0]));")

;; Pattern E (ashiftrt variant): HI register signed with const_int shift
(define_peephole2
  [(set (match_operand:HI 0 "register_operand")
	(ashiftrt:HI (match_dup 0) (match_operand:SI 2 "const_int_operand")))
   (set (match_dup 0)
	(and:HI (match_dup 0) (const_int 1)))]
  "flag_m68k_btst_extract && TUNE_68000_10 && optimize_insn_for_speed_p ()
   && IN_RANGE (INTVAL (operands[2]), 4, 15)"
...
;; Pattern F: HI register unsigned with register shift count (3->2 RTL insns)
;; Matches: moveq #N,dX; lsr.w dX,dY; and.w #1,dY -> btst #N,dY; sne; neg
;; For shift counts 9-15 which exceed 68000's immediate shift limit (1-8).
(define_peephole2
  [(set (match_operand:HI 4 "register_operand")
	(match_operand 5 "const_int_operand"))
   (set (match_operand:HI 0 "register_operand")
	(lshiftrt:HI (match_dup 0) (match_dup 4)))
   (set (match_dup 0)
	(and:HI (match_dup 0) (const_int 1)))]
  "flag_m68k_btst_extract && TUNE_68000_10 && optimize_insn_for_speed_p ()
   && IN_RANGE (INTVAL (operands[5]), 9, 15)
   && peep2_reg_dead_p (3, operands[4])"
  [(set (match_dup 2)
	(ne:QI (zero_extract:SI (match_dup 3) (const_int 1)
				(minus:SI (const_int 31) (match_dup 5)))
	       (const_int 0)))
   (set (match_dup 2) (neg:QI (match_dup 2)))]
  "operands[2] = gen_rtx_REG (QImode, REGNO (operands[0]));
   operands[3] = gen_rtx_REG (SImode, REGNO (operands[0]));")

;; Pattern F (ashiftrt variant): HI register signed with register shift count
(define_peephole2
  [(set (match_operand:HI 4 "register_operand")
	(match_operand 5 "const_int_operand"))
   (set (match_operand:HI 0 "register_operand")
	(ashiftrt:HI (match_dup 0) (match_dup 4)))
   (set (match_dup 0)
	(and:HI (match_dup 0) (const_int 1)))]
  "flag_m68k_btst_extract && TUNE_68000_10 && optimize_insn_for_speed_p ()
   && IN_RANGE (INTVAL (operands[5]), 9, 15)
...
;; Note on ZERO_EXTRACT/SIGN_EXTRACT patterns for 68000:
;; GCC does NOT generate ZERO_EXTRACT or SIGN_EXTRACT patterns for
;; word-aligned 16-bit extractions.  Instead it uses:
;;   - lshiftrt:SI by 16 for unsigned high word (matched by lshrsi_16)
;;   - ashiftrt:SI by 16 for signed high word (matched by ashrsi_16)
;;   - and:SI with 0xFFFF for low word extraction
;;   - subreg:HI for direct low word access
;; ZERO_EXTRACT is only generated for non-aligned bitfield access which
;; requires 68020+ bitfield instructions (bfextu/bfexts).
;; The m68k_highword_opt pass handles post-compilation optimization of
;; patterns like "clr.w; swap" -> "swap" when upper bits don't matter.
...
;; Convert moveq #1 + lsl.l to moveq #0 + bset for standalone 1 << n.
;; On 68000, lsl.l Dn,Dm costs 8+2*n cycles (up to 70 for n=31).
;; bset Dn,Dm costs 8 cycles constant.  Saves 2*n cycles.
(define_peephole2
  [(set (match_operand:SI 0 "register_operand")
	(const_int 1))
   (set (match_dup 0)
  ...
  "TUNE_68000_10
   && DATA_REG_P (operands[0])
   && DATA_REG_P (operands[1])
   && REGNO (operands[0]) != REGNO (operands[1])"
  [(set (match_dup 0) (const_int 0))
   (set (match_dup 0)
	(ior:SI (ashift:SI (const_int 1)
			   (and:SI (match_dup 1) (const_int 31)))
		(match_dup 0)))])

;; HImode variant for -mshort: moveq #1 + lsl.w to moveq #0 + bset.
;; bset on data registers is always 32-bit; widening to SImode is safe
;; because moveq #0 clears all 32 bits and the set bit is in [0,15].
(define_peephole2
  [(set (match_operand:HI 0 "register_operand")
	(const_int 1))
   (set (match_dup 0)
  ...
  [(set (match_dup 2) (const_int 0))
   (set (match_dup 2)
	(ior:SI (ashift:SI (const_int 1)
			   (and:SI (match_dup 3) (const_int 31)))
		(match_dup 2)))]
{
...
;; Convert move #POW2 + lsr.l to moveq + sub + moveq #0 + bset + move.
;; Replaces expensive variable shift (8+2*n cycles) with constant-time
;; bset (8 cycles).  Requires the shift register to be dead so we can
;; reuse it for the bset destination.
;;
;; Before: move.l #POW2,d0; lsr.l d1,d0   (20+2*shift cycles)
;; After:  moveq #log2,d0; sub.l d1,d0; moveq #0,d1;
;;         bset d0,d1; move.l d1,d0        (28 cycles constant)
(define_peephole2
  [(set (match_operand:SI 0 "register_operand")
  ...
  "TUNE_68000_10
   ...
   && exact_log2 (INTVAL (operands[1]) & 0xffffffff) > 0
  ...
  [(set (match_dup 0) (match_dup 3))
   ...
   (set (match_dup 2)
	(ior:SI (ashift:SI (const_int 1)
			   (and:SI (match_dup 0) (const_int 31)))
		(match_dup 2)))
   (set (match_dup 0) (match_dup 2))]
{
  operands[3] = GEN_INT (exact_log2 (INTVAL (operands[1]) & 0xffffffff));
})

;; HImode variant: move.w #POW2 + lsr.w to moveq + sub + moveq #0 + bset.
(define_peephole2
  [(set (match_operand:HI 0 "register_operand")
  ...
  "TUNE_68000_10
   ...
   && DATA_REG_P (operands[2])
   && REGNO (operands[0]) != REGNO (operands[2])
   && exact_log2 (INTVAL (operands[1]) & 0xffff) > 0
   && peep2_reg_dead_p (2, operands[2])"
  [(set (match_dup 4) (match_dup 3))
   (set (match_dup 4) (minus:SI (match_dup 4) (match_dup 5)))
   (set (match_dup 5) (const_int 0))
   (set (match_dup 5)
	(ior:SI (ashift:SI (const_int 1)
			   (and:SI (match_dup 4) (const_int 31)))
		(match_dup 5)))
   (set (match_dup 4) (match_dup 5))]
{
  operands[3] = GEN_INT (exact_log2 (INTVAL (operands[1]) & 0xffff));
  operands[4] = gen_rtx_REG (SImode, REGNO (operands[0]));
...
;; Combine may merge a subreg:HI extraction into a shift+mask, producing
;; zero_extract:HI on the low half of a 32-bit register.  Map this to
;; bfextu on the full register with adjusted bit position (+16).
(define_insn "*extv_bfextu_reg_hi"
  [(set (match_operand:HI 0 "register_operand" "=d")
	(zero_extract:HI
	  (subreg:HI (match_operand:SI 1 "register_operand" "d") 2)
	  (match_operand:SI 2 "const_int_operand" "n")
	  (match_operand:SI 3 "const_int_operand" "n")))]
  "TARGET_68020 && TARGET_BITFIELD
   && IN_RANGE (INTVAL (operands[3]) + 16, 0, 31)"
{
...
;; Tablejump data load: reads an offset from the jump table.
;; Using UNSPEC instead of plain MEM avoids LRA needing an ADDR_REG
;; for the index register.  In the (d8,PC,Xn) addressing mode, any
;; register class works for Xn.  A pre-IRA define_split converts
;; (mem (plus reg (label_ref))) to this form.

;; Long index variant: (plus:SI (reg:SI) (label_ref))
;; Uses .l index extension.  Generated when the index is 32-bit (no -mshort
;; or when the shift to scale the index is done in SImode).

(define_insn "*tablejump_load_hi"
  [(set (match_operand:HI 0 "register_operand" "=r")
	(unspec:HI [(match_operand:SI 1 "register_operand" "r")
		    (label_ref (match_operand 2 "" ""))]
		   UNSPEC_TABLEJUMP_LOAD))]
  "!flag_pic"
{
  static char buf[80];
  char label_buf[32];
  ...
  if (*label == '*')
    label++;
  if (MOTOROLA)
    sprintf (buf, "move.w %s(%%%%pc,%%1.l),%%0", label);
  else
    sprintf (buf, "movew %%%%pc@(%s,%%1:l),%%0", label);
  return buf;
}
  [(set_attr "type" "move")])

(define_insn "*tablejump_load_si"
  ...
  "!flag_pic"
{
  ...
  if (*label == '*')
    label++;
  if (MOTOROLA)
    sprintf (buf, "move.l %s(%%%%pc,%%1.l),%%0", label);
  else
    sprintf (buf, "movel %%%%pc@(%s,%%1:l),%%0", label);
  return buf;
}
  [(set_attr "type" "move_l")])

;; Word index variant: (plus:SI (sign_extend:SI (reg:HI)) (label_ref))
;; Uses .w index extension.  Generated with -mshort when the index
;; scaling shift is done in HImode, requiring sign_extend for the address.

(define_insn "*tablejump_load_hi_w"
  [(set (match_operand:HI 0 "register_operand" "=r")
	(unspec:HI [(sign_extend:SI
  ...
  "!flag_pic"
{
  ...
  if (*label == '*')
    label++;
  if (MOTOROLA)
    sprintf (buf, "move.w %s(%%%%pc,%%1.w),%%0", label);
  else
    sprintf (buf, "movew %%%%pc@(%s,%%1:w),%%0", label);
  return buf;
}
  [(set_attr "type" "move")])

(define_insn "*tablejump_load_si_w"
  [(set (match_operand:SI 0 "register_operand" "=r")
	(unspec:SI [(sign_extend:SI
		      (match_operand:HI 1 "register_operand" "r"))
		    (label_ref (match_operand 2 "" ""))]
		   UNSPEC_TABLEJUMP_LOAD))]
  "!flag_pic"
{
  ...
  if (*label == '*')
    label++;
  if (MOTOROLA)
    sprintf (buf, "move.l %s(%%%%pc,%%1.w),%%0", label);
  else
    sprintf (buf, "movel %%%%pc@(%s,%%1:w),%%0", label);
  return buf;
}
  [(set_attr "type" "move_l")])

;; Pre-IRA splits: convert tablejump loads from plain MEM to UNSPEC form.
;; This lets IRA see "=r" (any register) instead of requiring ADDR_REGS
;; for the index, avoiding unnecessary callee-save spills.
;;
;; Long index (SI register) — prefer .w index when table is small enough.
;; The .w form keeps the index computation in HImode, saving cycles on 68000
;; (and.w/add.w are faster than and.l/add.l).
;; The prepare block also narrows the preceding index scaling insn (e.g.
;; add.l → add.w) when the index is known to fit in 16 bits.
(define_split
  [(set (match_operand:HI 0 "register_operand")
	(mem:HI (plus:SI (match_operand:SI 1 "register_operand")
			 (label_ref (match_operand 2 "")))))]
  "!reload_completed && !flag_pic
   && !TARGET_COLDFIRE
   && m68k_tablejump_fits_word_index_p (operands[2])"
  [(set (match_dup 0)
	(unspec:HI [(sign_extend:SI (match_dup 3))
		    (label_ref (match_dup 2))]
		   UNSPEC_TABLEJUMP_LOAD))]
{
  operands[3] = gen_lowpart (HImode, operands[1]);

  /* Try to narrow the preceding index scaling insn to HImode.
     After a range check (cmp + jhi), the index is known to be small.
     Patterns: (plus:SI reg reg) = multiply by 2,
               (ashift:SI reg const) = multiply by power of 2.
     Source and dest may be different registers.  */
  ...
  if (def_ref && !DF_REF_NEXT_REG (def_ref))
	...
	{
	  rtx src = SET_SRC (set);
	  rtx hi_dest = gen_lowpart (HImode, SET_DEST (set));
	  rtx new_src = NULL_RTX;

	  /* (plus:SI rA rA) → (plus:HI lo_rA lo_rA) */
	  if (GET_CODE (src) == PLUS
	      && REG_P (XEXP (src, 0)) && REG_P (XEXP (src, 1))
	      && REGNO (XEXP (src, 0)) == REGNO (XEXP (src, 1)))
	    {
	      rtx hi_op0 = gen_lowpart (HImode, XEXP (src, 0));
	      rtx hi_op1 = copy_rtx (hi_op0);
	      new_src = gen_rtx_PLUS (HImode, hi_op0, hi_op1);
	    }
	  /* (ashift:SI rA const) → (ashift:HI lo_rA const) */
	  else if (GET_CODE (src) == ASHIFT
		   && REG_P (XEXP (src, 0))
		   && CONST_INT_P (XEXP (src, 1)))
	    {
	      rtx hi_op = gen_lowpart (HImode, XEXP (src, 0));
	      new_src = gen_rtx_ASHIFT (HImode, hi_op, XEXP (src, 1));
	    }

	  if (new_src)
	    {
	      rtx new_pat = gen_rtx_SET (hi_dest, new_src);
	      if (validate_change (def_insn, &PATTERN (def_insn),
				   new_pat, true))
		{
		  confirm_change_group ();
		  INSN_CODE (def_insn) = -1;
		}
	      else
		cancel_changes (0);
	    }
...
;; Fallback: .l index for large tables or ColdFire.
(define_split
  [(set (match_operand:HI 0 "register_operand")
	(mem:HI (plus:SI (match_operand:SI 1 "register_operand")
			 (label_ref (match_operand 2 "")))))]
  "!reload_completed && !flag_pic"
  [(set (match_dup 0)
	(unspec:HI [(match_dup 1) (label_ref (match_dup 2))]
		   UNSPEC_TABLEJUMP_LOAD))])
...
(define_split
  ...
  "!reload_completed && !flag_pic"
  [(set (match_dup 0)
	(unspec:SI [(match_dup 1) (label_ref (match_dup 2))]
		   UNSPEC_TABLEJUMP_LOAD))])

;; Word index (HI register, sign-extended for addressing):
(define_split
  [(set (match_operand:HI 0 "register_operand")
	(mem:HI (plus:SI (sign_extend:SI
			   (match_operand:HI 1 "register_operand"))
			 (label_ref (match_operand 2 "")))))]
  "!reload_completed && !flag_pic"
  [(set (match_dup 0)
	(unspec:HI [(sign_extend:SI (match_dup 1))
		    (label_ref (match_dup 2))]
		   UNSPEC_TABLEJUMP_LOAD))])

(define_split
  [(set (match_operand:SI 0 "register_operand")
	(mem:SI (plus:SI (sign_extend:SI
			   (match_operand:HI 1 "register_operand"))
			 (label_ref (match_operand 2 "")))))]
  "!reload_completed && !flag_pic"
  [(set (match_dup 0)
	(unspec:SI [(sign_extend:SI (match_dup 1))
		    (label_ref (match_dup 2))]
		   UNSPEC_TABLEJUMP_LOAD))])
...
;; On non-ColdFire targets, HImode decrement-and-branch is handled
;; exclusively by doloop_end_hi (which has "+d" constraint, forcing a data
;; register for dbra).  *dbne_hi is ColdFire-only (subq/jcc fallback,
;; no dbra available).
(define_insn "*dbne_hi"
  [(set (pc)
	(if_then_else
...
(define_insn "*dbge_hi"
  [(set (pc)
	(if_then_else
	  (ge (plus:HI (match_operand:HI 0 "nonimmediate_operand" "+d,a,m")
		       (const_int -1))
	      (const_int 0))
	  (label_ref (match_operand 1 "" ",,"))
	  (pc)))
...
;; DOLOOP patterns for integration with GCC's loop-doloop pass.
;; These patterns are enabled via TARGET_CAN_USE_DOLOOP_P hook.
...
(define_expand "doloop_end"
  ...
  "!TARGET_COLDFIRE"
{
  /* Only use dbra for HImode counters.  SImode counters require the
     extended 4-instruction sequence which is rarely beneficial.
     Let other patterns handle SImode loop optimization.  */
  if (GET_MODE (operands[0]) == HImode)
    {
      /* RTL safety net: block doloop when the exit IV has body uses
	 AND register pressure is high.  Body uses mean the doloop
	 counter is redundant; high pressure means the extra register
	 will cause spills to address registers where dbra cannot
	 operate.  */
      if (m68k_doloop_exit_iv_has_body_uses (operands[1]))
	FAIL;
      emit_jump_insn (gen_doloop_end_hi (operands[0], operands[1]));
      DONE;
    }
  FAIL;
})

;; HImode doloop_end - generates dbra directly
(define_insn "doloop_end_hi"
  [(set (pc)
	(if_then_else
...
(define_insn "*dbge_si"
  [(set (pc)
	(if_then_else
	  (ge (plus:SI (match_operand:SI 0 "nonimmediate_operand" "+d,a,m")
		       (const_int -1))
	      (const_int 0))
	  (label_ref (match_operand 1 "" ",,"))
	  (pc)))
...
(define_insn "indirect_jump"
  ...
  [(set_attr "type" "jmp")])

;; Non-68020 indexed LEA with conditional splitting.
;;
;; On 68000 and ColdFire, indexed addressing (d8,An,Xn) only supports
;; 8-bit signed displacement.  After LRA eliminates the frame pointer
;; the displacement can exceed this limit, causing an ICE because the
;; *lea pattern's "p" constraint becomes unsatisfiable.
;;
;; These patterns use register/const constraints instead of "p", giving
;; LRA satisfiable constraints.  When the displacement fits in 8 bits,
;; the output template emits a single LEA.  When it doesn't (after fp
;; elimination), the post-reload split decomposes it into two insns.

(define_insn_and_split "*lea_indexed_disp_scaled"
  [(set (match_operand:SI 0 "register_operand" "=a")
	(plus:SI
	  (plus:SI
  ...
  "(TARGET_COLDFIRE || TARGET_68020) && IN_RANGE (INTVAL (operands[2]), 1, 3)"
{
  if (!IN_RANGE (INTVAL (operands[4]), -0x80, 0x7f))
    return "#";

  operands[5] = GEN_INT (1 << INTVAL (operands[2]));
  if (INTVAL (operands[4]) == 0)
    return MOTOROLA ? "lea (%3,%1.l*%c5),%0" : "lea %3@(0,%1:l:%c5),%0";
  else
    return MOTOROLA ? "lea (%c4,%3,%1.l*%c5),%0" : "lea %3@(%c4,%1:l:%c5),%0";
}
  "&& reload_completed
   && !IN_RANGE (INTVAL (operands[4]), -0x80, 0x7f)"
  [(set (match_dup 0)
...
(define_insn_and_split "*lea_indexed_disp"
  [(set (match_operand:SI 0 "register_operand" "=a,a")
	(plus:SI
	  (plus:SI
	    (match_operand:SI 1 "register_operand" "a,d")
	    (match_operand:SI 2 "register_operand" "da,a"))
	  (match_operand:SI 3 "const_int_operand" "n,n")))]
  "!TARGET_68020"
{
  if (!IN_RANGE (INTVAL (operands[3]), -0x80, 0x7f))
    return "#";

  /* Identify base (address register) and index for assembly output.  */
  operands[4] = ADDRESS_REG_P (operands[1]) ? operands[1] : operands[2];
  operands[5] = ADDRESS_REG_P (operands[1]) ? operands[2] : operands[1];

  if (INTVAL (operands[3]) == 0)
    return MOTOROLA ? "lea (%4,%5.l),%0" : "lea %4@(0,%5:l),%0";
  else
    return MOTOROLA ? "lea (%c3,%4,%5.l),%0" : "lea %4@(%c3,%5:l),%0";
}
  "&& reload_completed
   && !IN_RANGE (INTVAL (operands[3]), -0x80, 0x7f)"
  [(set (match_dup 0) (plus:SI (match_dup 4) (match_dup 5)))
   (set (match_dup 0) (plus:SI (match_dup 0) (match_dup 3)))]
  {
    /* Identify base (address register) and index for the split.  */
    if (ADDRESS_REG_P (operands[1]))
      {
	operands[4] = operands[1];
...
;; LEA with scaled index for 68020+ (base + index * scale).
;; Matches (plus base (ashift dreg shift)) and (plus (ashift dreg shift) base).
;; Must appear before *lea so it takes priority for scaled index forms.

(define_insn "*lea_scaled"
  [(set (match_operand:SI 0 "register_operand" "=a")
	(plus:SI (match_operand:SI 1 "register_operand" "a")
		 (ashift:SI (match_operand:SI 2 "register_operand" "da")
			    (match_operand:SI 3 "const_int_operand" "n"))))]
  "TARGET_68020 && IN_RANGE (INTVAL (operands[3]), 1, 3)"
{
  operands[4] = GEN_INT (1 << INTVAL (operands[3]));
  return MOTOROLA ? "lea (%1,%2.l*%c4),%0" : "lea %1@(0,%2:l:%c4),%0";
}
  [(set_attr "type" "lea")])

(define_insn "*lea_scaled_rev"
  [(set (match_operand:SI 0 "register_operand" "=a")
	(plus:SI (ashift:SI (match_operand:SI 1 "register_operand" "da")
			    (match_operand:SI 2 "const_int_operand" "n"))
		 (match_operand:SI 3 "register_operand" "a")))]
  "TARGET_68020 && IN_RANGE (INTVAL (operands[2]), 1, 3)"
{
  operands[4] = GEN_INT (1 << INTVAL (operands[2]));
  return MOTOROLA ? "lea (%3,%1.l*%c4),%0" : "lea %3@(0,%1:l:%c4),%0";
}
  [(set_attr "type" "lea")])
...
;; On 68000/68010, testing an address register against zero requires
;; cmp.w #0,%aN (4 bytes, 12 cycles) because tst.l doesn't work on
;; address registers.  When a scratch data register is available, add
;; a clobber to form a *cbranchsi4_areg_zero insn that outputs
;; move.l %aN,%dN (2 bytes, 4 cycles) with CC elision.  The parallel
;; keeps this as a single insn so cprop_hardreg cannot decompose it.
(define_peephole2
  [(match_scratch:SI 4 "d")
  ...
  "!TARGET_68020 && !TARGET_COLDFIRE
   ...
   && (operands[2] == pc_rtx || operands[3] == pc_rtx)"
  [(parallel
    [(set (pc) (if_then_else (match_op_dup 0 [(match_dup 1) (const_int 0)])
                              (match_dup 2)
                              (match_dup 3)))
     (clobber (match_dup 4))])])
...
;; ============================================================
;; Read-modify-write with post-increment peepholes
;; ============================================================

;; Collapse: load (An) -> op Dn -> store (An)+ into: OP.x Dn,(An)+
;; Standard order: (op loaded_reg, src_reg)
(define_peephole2
  [(set (match_operand:I 0 "register_operand")
        (mem:I (match_operand:SI 1 "register_operand")))
   (set (match_dup 0)
        (rmw_op:I (match_dup 0)
                  (match_operand:I 2 "register_operand")))
   (set (mem:I (post_inc:SI (match_dup 1)))
        (match_dup 0))]
  "reload_completed
   && !TARGET_COLDFIRE
   && DATA_REG_P (operands[2])
  ...
  [(set (mem:I (post_inc:SI (match_dup 1)))
        (rmw_op:I (mem:I (match_dup 1))
                  (match_dup 2)))])

;; Reversed commutative: (op src_reg, loaded_reg)
(define_peephole2
  [(set (match_operand:I 0 "register_operand")
        (mem:I (match_operand:SI 1 "register_operand")))
   (set (match_dup 0)
        (rmw_comm_op:I (match_operand:I 2 "register_operand")
                       (match_dup 0)))
   (set (mem:I (post_inc:SI (match_dup 1)))
        (match_dup 0))]
  "reload_completed
   ...
   && peep2_reg_dead_p (3, operands[0])"
  [(set (mem:I (post_inc:SI (match_dup 1)))
        (rmw_comm_op:I (mem:I (match_dup 1))
                       (match_dup 2)))])

;; LRA copy-op-store patterns: copy Dn -> temp; temp op= (An); (An)+ = temp
;; LRA decomposes memory-destination ops differently from legacy reload:
;; it copies the register source first, then adds from memory, instead of
;; loading from memory first.  Only applies to commutative operations since
;; non-commutative ops (minus) would load from memory (covered above).
;; Standard order: (op temp, mem)
(define_peephole2
  [(set (match_operand:I 0 "register_operand")
        (match_operand:I 2 "register_operand"))
   (set (match_dup 0)
        (rmw_comm_op:I (match_dup 0)
                       (mem:I (match_operand:SI 1 "register_operand"))))
   (set (mem:I (post_inc:SI (match_dup 1)))
        (match_dup 0))]
  "reload_completed
...
;; Reversed commutative: (op mem, temp)
(define_peephole2
  [(set (match_operand:I 0 "register_operand")
        (match_operand:I 2 "register_operand"))
   (set (match_dup 0)
        (rmw_comm_op:I (mem:I (match_operand:SI 1 "register_operand"))
                       (match_dup 0)))
   (set (mem:I (post_inc:SI (match_dup 1)))
        (match_dup 0))]
  "reload_completed
   ...
   && peep2_reg_dead_p (3, operands[0])"
  [(set (mem:I (post_inc:SI (match_dup 1)))
        (rmw_comm_op:I (match_dup 2)
                       (mem:I (match_dup 1))))])

;; Collapse: OP.x Dn,(An) + addq #size,An  into: OP.x Dn,(An)+
;; When the RMW is already combined (no separate load/store), auto_inc_dec
;; cannot add POST_INC because the address register appears twice in the
;; pattern (src MEM and dest MEM).  This 2-insn peephole handles it.
;; Standard order: (op mem, src_reg)
(define_peephole2
  [(set (mem:I (match_operand:SI 1 "register_operand"))
        (rmw_op:I (mem:I (match_dup 1))
                  (match_operand:I 2 "register_operand")))
   (set (match_dup 1)
        (plus:SI (match_dup 1)
...
;; Reversed commutative: (op src_reg, mem)
(define_peephole2
  [(set (mem:I (match_operand:SI 1 "register_operand"))
        (rmw_comm_op:I (match_operand:I 2 "register_operand")
                       (mem:I (match_dup 1))))
...
;; ============================================================
;; Combine adjacent post-increment operations to use larger modes
;; ============================================================

;; Combine: move.b #x,(a0)+ + move.b #y,(a0)+ -> move.w #xy,(a0)+
;; Only safe on 68020+ due to alignment requirements on 68000/68010
(define_peephole2
  [(set (mem:QI (post_inc:SI (match_operand:SI 0 "register_operand")))
        (match_operand:QI 1 "const_int_operand"))
   (set (mem:QI (post_inc:SI (match_dup 0)))
        (match_operand:QI 2 "const_int_operand"))]
  "reload_completed && !TUNE_68000_10"
  [(set (mem:HI (post_inc:SI (match_dup 0))) (match_dup 3))]
{
  operands[3] = GEN_INT ((INTVAL (operands[1]) << 8)
                         | (INTVAL (operands[2]) & 0xff));
})

;; Combine: move.b #x,(a0)+ + move.b #y,(a0) -> move.w #xy,(a0)
;; When register is dead after, we can drop the final increment
(define_peephole2
  [(set (mem:QI (post_inc:SI (match_operand:SI 0 "register_operand")))
        (match_operand:QI 1 "const_int_operand"))
...
;; Combine: move.w #x,(a0)+ + move.w #y,(a0)+ -> move.l #xy,(a0)+
;; Safe on all m68k since word access already ensures alignment
(define_peephole2
  ...
  "reload_completed"
  [(set (mem:SI (post_inc:SI (match_dup 0))) (match_dup 3))]
{
...
;; Combine: move.b (a1)+,(a0)+ + move.b (a1)+,(a0)+ -> move.w (a1)+,(a0)+
;; Memory-to-memory with post-increment on both sides
(define_peephole2
  [(set (mem:QI (post_inc:SI (match_operand:SI 0 "register_operand")))
...
;; Combine: move.b (a1)+,(a0)+ + move.b (a1),(a0) -> move.w (a1),(a0)
;; When both registers are dead after
(define_peephole2
  [(set (mem:QI (post_inc:SI (match_operand:SI 0 "register_operand")))
        (mem:QI (post_inc:SI (match_operand:SI 1 "register_operand"))))
   (set (mem:QI (match_dup 0))
...
;; Combine: move.w (a1)+,(a0)+ + move.w (a1)+,(a0)+ -> move.l (a1)+,(a0)+
(define_peephole2
  [(set (mem:HI (post_inc:SI (match_operand:SI 0 "register_operand")))
...
;; ============================================================
;; Combine adjacent offset-based accesses: n(a0) + n+size(a0)
;; ============================================================

;; Combine: move.b #x,n(a0) + move.b #y,n+1(a0) -> move.w #xy,n(a0)
;; For 68000/68010: only when n is even (word-aligned)
;; For 68020+: always safe
(define_peephole2
  [(set (mem:QI (plus:SI (match_operand:SI 0 "register_operand")
                         (match_operand:SI 1 "const_int_operand")))
        (match_operand:QI 2 "const_int_operand"))
   (set (mem:QI (plus:SI (match_dup 0)
                         (match_operand:SI 3 "const_int_operand")))
        (match_operand:QI 4 "const_int_operand"))]
  "reload_completed
   && INTVAL (operands[3]) == INTVAL (operands[1]) + 1
   && (!TUNE_68000_10 || (INTVAL (operands[1]) & 1) == 0)"
  [(set (mem:HI (plus:SI (match_dup 0) (match_dup 1))) (match_dup 5))]
{
  operands[5] = GEN_INT ((INTVAL (operands[2]) << 8)
                         | (INTVAL (operands[4]) & 0xff));
})

;; Combine: move.w #x,n(a0) + move.w #y,n+2(a0) -> move.l #xy,n(a0)
;; Always safe since word access already ensures alignment
(define_peephole2
  ...
  [(set (mem:SI (plus:SI (match_dup 0) (match_dup 1))) (match_dup 5))]
{
  operands[5] = GEN_INT ((INTVAL (operands[2]) << 16)
                         | (INTVAL (operands[4]) & 0xffff));
})

;; Combine: move.b #x,(a0) + move.b #y,1(a0) -> move.w #xy,(a0)
;; Variant where first access has zero offset
(define_peephole2
  [(set (mem:QI (match_operand:SI 0 "register_operand"))
        (match_operand:QI 1 "const_int_operand"))
   (set (mem:QI (plus:SI (match_dup 0) (const_int 1)))
        (match_operand:QI 2 "const_int_operand"))]
  "reload_completed && !TUNE_68000_10"
  [(set (mem:HI (match_dup 0)) (match_dup 3))]
{
...
;; Combine: move.w #x,(a0) + move.w #y,2(a0) -> move.l #xy,(a0)
;; Variant where first access has zero offset
(define_peephole2
  [(set (mem:HI (match_operand:SI 0 "register_operand"))
        (match_operand:HI 1 "const_int_operand"))
   (set (mem:HI (plus:SI (match_dup 0) (const_int 2)))
        (match_operand:HI 2 "const_int_operand"))]
  "reload_completed"
  [(set (mem:SI (match_dup 0)) (match_dup 3))]
{
...
;; ============================================================
;; Combine adjacent CLR instructions
;; ============================================================

;; Combine: clr.b n(a0) + clr.b n+1(a0) -> clr.w n(a0)
(define_peephole2
  [(set (mem:QI (plus:SI (match_operand:SI 0 "register_operand")
                         (match_operand:SI 1 "const_int_operand")))
        (const_int 0))
  ...
  "reload_completed
   ...
   && (!TUNE_68000_10 || (INTVAL (operands[1]) & 1) == 0)"
  [(set (mem:HI (plus:SI (match_dup 0) (match_dup 1))) (const_int 0))])

;; Combine: clr.w n(a0) + clr.w n+2(a0) -> clr.l n(a0)
(define_peephole2
  [(set (mem:HI (plus:SI (match_operand:SI 0 "register_operand")
                         (match_operand:SI 1 "const_int_operand")))
        (const_int 0))
   (set (mem:HI (plus:SI (match_dup 0)
        ...
        (const_int 0))]
  "reload_completed
   && INTVAL (operands[2]) == INTVAL (operands[1]) + 2"
  [(set (mem:SI (plus:SI (match_dup 0) (match_dup 1))) (const_int 0))])

;; Combine: clr.b (a0) + clr.b 1(a0) -> clr.w (a0)
(define_peephole2
  [(set (mem:QI (match_operand:SI 0 "register_operand"))
        (const_int 0))
   (set (mem:QI (plus:SI (match_dup 0) (const_int 1)))
        (const_int 0))]
  "reload_completed && !TUNE_68000_10"
  [(set (mem:HI (match_dup 0)) (const_int 0))])

;; Combine: clr.w (a0) + clr.w 2(a0) -> clr.l (a0)
(define_peephole2
  [(set (mem:HI (match_operand:SI 0 "register_operand"))
        (const_int 0))
   (set (mem:HI (plus:SI (match_dup 0) (const_int 2)))
        (const_int 0))]
  "reload_completed"
  [(set (mem:SI (match_dup 0)) (const_int 0))])

;; Combine: clr.b (a0)+ + clr.b (a0)+ -> clr.w (a0)+
(define_peephole2
  [(set (mem:QI (post_inc:SI (match_operand:SI 0 "register_operand")))
        (const_int 0))
   (set (mem:QI (post_inc:SI (match_dup 0)))
        (const_int 0))]
  "reload_completed && !TUNE_68000_10"
  [(set (mem:HI (post_inc:SI (match_dup 0))) (const_int 0))])

;; Combine: clr.w (a0)+ + clr.w (a0)+ -> clr.l (a0)+
(define_peephole2
  [(set (mem:HI (post_inc:SI (match_operand:SI 0 "register_operand")))
        (const_int 0))
   (set (mem:HI (post_inc:SI (match_dup 0)))
        (const_int 0))]
  "reload_completed"
  [(set (mem:SI (post_inc:SI (match_dup 0))) (const_int 0))])

;; ============================================================
;; Multiply and sign-extension peepholes
;; ============================================================

;; Combine 16-bit signed multiply with sign extension to 32-bit multiply.
;; muls.w already produces a 32-bit signed result, so the ext.l is redundant.
;; This converts: muls.w %d1,%d0; ext.l %d0  ->  muls.w %d1,%d0
;; Note: Changes behavior when 16-bit multiply would overflow (undefined in C).
(define_peephole2
  [(set (match_operand:HI 0 "register_operand")
        (mult:HI (match_dup 0)
                 (match_operand:HI 1 "general_operand")))
  ...
  "REGNO (operands[0]) == REGNO (operands[2])"
  [(set (match_dup 2)
        (mult:SI (sign_extend:SI (match_dup 3))
                 (sign_extend:SI (match_dup 1))))]
{
  operands[3] = gen_rtx_REG (HImode, REGNO (operands[2]));
})

;; Same pattern but with operands swapped (commutative)
(define_peephole2
  [(set (match_operand:HI 0 "register_operand")
...
;; ============================================================
;; ANDI + ORI folding peepholes
;; ============================================================

;; Combine andi.l #$ffff + ori.l #xxxx0000 + move.w #yyyy into move.l #xxxxyyyy
;; The sequence clears upper word, sets it via ORI, then sets lower word.
;; Final result is a constant that can be loaded directly.
;; Saves 10 bytes (16 -> 6) and ~28 cycles.
(define_peephole2
  [(set (match_operand:SI 0 "register_operand")
        (and:SI (match_dup 0)
                (const_int 65535)))
   (set (match_dup 0)
        (ior:SI (match_dup 0)
                (match_operand:SI 1 "const_int_operand")))
   (set (strict_low_part (match_operand:HI 2 "register_operand"))
...
{
  HOST_WIDE_INT upper = INTVAL (operands[1]) & 0xffff0000;
  HOST_WIDE_INT lower = INTVAL (operands[3]) & 0xffff;
  operands[4] = GEN_INT (upper | lower);
})

;; Combine andi.l #$ffff + ori.l #xxxx0000 into swap + move.w #xxxx + swap
;; Saves 4 bytes (12 -> 8) and ~16 cycles.
(define_peephole2
  [(set (match_operand:SI 0 "register_operand")
        (and:SI (match_dup 0)
                (const_int 65535)))
   ...
   (set (match_dup 0) (rotate:SI (match_dup 0) (const_int 16)))]
{
  ...
  operands[3] = GEN_INT ((INTVAL (operands[1]) >> 16) & 0xffff);
})

;; NOTE: The mulhi3 pattern uses register_operand for operand 1 (not
;; general_operand) to prevent combine/fwprop from merging memory loads
;; into the multiply.  This keeps register pressure visible to IRA and
;; avoids unnecessary LRA spills in tight loops (e.g. test_matrix_mul).

;; POST_INC reconstruction for LRA-decomposed patterns is handled by
;; try_merge_postinc in m68k_pass_normalize_autoinc (m68k-rtl-passes.cc),
;; which generalizes to any instruction type and mode size.

;; ============================================================
;; Eliminate register intermediate in mem-to-mem copies
;; ============================================================
;; When a data register is loaded from memory and immediately stored
;; to memory, and the register is dead afterward, replace the two
;; instructions with a direct mem-to-mem move.  Not valid on ColdFire
;; which lacks mem-to-mem moves.

;; 2-insn: load reg,(mem) + store (mem),reg -> move (mem),(mem)
;; Register must be dead after the store.
(define_peephole2
  [(set (match_operand:I 0 "register_operand")
        (match_operand:I 1 "memory_operand"))
   (set (match_operand:I 2 "memory_operand")
        (match_dup 0))]
  "reload_completed
   && !TARGET_COLDFIRE
   && DATA_REG_P (operands[0])
...
;; 3-insn: load reg,(mem) + store (mem),reg + cbranch reg
;; -> combined mem-to-mem move + branch on CC from the move.
;; The register is used only as intermediate + branch test, and is
;; dead after the branch.  The combined insn (*mov<mode>_mem_cbranch)
;; emits the move and branch without an explicit tst/cmp.
;; The (set (pc) ...) must be the first PARALLEL element so that
;; pc_set() and any_condjump_p() recognize it as a conditional jump.
(define_peephole2
  [(set (match_operand:I 0 "register_operand")
        (match_operand:I 1 "memory_operand"))
   (set (match_operand:I 2 "memory_operand")
  ...
  [(parallel
     [(set (pc)
           (if_then_else (match_op_dup 3 [(match_dup 0) (const_int 0)])
                         (label_ref (match_dup 4))
                         (pc)))
      (set (match_dup 2) (match_dup 1))
      (clobber (match_dup 0))])])

;; Reversed branch variant.
(define_peephole2
  [(set (match_operand:I 0 "register_operand")
        (match_operand:I 1 "memory_operand"))
   (set (match_operand:I 2 "memory_operand")
...
;; ============================================================
;; Combine adjacent offset-based memory-to-memory accesses
;; These patterns fire at peephole2 time before auto-inc synthesis
;; ============================================================

;; Combine: move.w (a0),(a1) + move.w 2(a0),2(a1) -> move.l (a0),(a1)
;; First access at register, second at register+2
(define_peephole2
  [(set (mem:HI (match_operand:SI 0 "register_operand"))
        (mem:HI (match_operand:SI 1 "register_operand")))
   (set (mem:HI (plus:SI (match_dup 0) (const_int 2)))
        (mem:HI (plus:SI (match_dup 1) (const_int 2))))]
  "reload_completed"
  [(set (mem:SI (match_dup 0)) (mem:SI (match_dup 1)))])

;; Combine: move.w n(a0),n(a1) + move.w n+2(a0),n+2(a1) -> move.l n(a0),n(a1)
;; Both accesses at register+offset
(define_peephole2
  [(set (mem:HI (plus:SI (match_operand:SI 0 "register_operand")
                         (match_operand:SI 2 "const_int_operand")))
        (mem:HI (plus:SI (match_operand:SI 1 "register_operand")
   ...
   (set (mem:HI (plus:SI (match_dup 0)
        ...
        (mem:HI (plus:SI (match_dup 1)
                         (match_dup 3))))]
  "reload_completed
   && INTVAL (operands[3]) == INTVAL (operands[2]) + 2"
  [(set (mem:SI (plus:SI (match_dup 0) (match_dup 2)))
        (mem:SI (plus:SI (match_dup 1) (match_dup 2))))])

;; Combine: move.w (a0),(a0,d0) + move.w 2(a0),2(a0,d0) -> move.l (a0),(a0,d0)
;; Indexed destination, register source
(define_peephole2
  [(set (mem:HI (plus:SI (match_operand:SI 0 "register_operand")
                         (match_operand:SI 1 "register_operand")))
        (mem:HI (match_operand:SI 2 "register_operand")))
   (set (mem:HI (plus:SI (plus:SI (match_dup 0) (match_dup 1))
                         (const_int 2)))
        (mem:HI (plus:SI (match_dup 2) (const_int 2))))]
  "reload_completed"
  [(set (mem:SI (plus:SI (match_dup 0) (match_dup 1)))
        (mem:SI (match_dup 2)))])

;; Combine: move.w n(a0),n(a0,d0) + move.w n+2(a0),n+2(a0,d0)
;;       -> move.l n(a0),n(a0,d0)
;; Indexed destination with offset, register source with offset
(define_peephole2
  [(set (mem:HI (plus:SI (plus:SI (match_operand:SI 0 "register_operand")
                                  (match_operand:SI 1 "register_operand"))
                         (match_operand:SI 4 "const_int_operand")))
        (mem:HI (plus:SI (match_operand:SI 2 "register_operand")
                         (match_dup 4))))
   (set (mem:HI (plus:SI (plus:SI (match_dup 0) (match_dup 1))
                         (match_operand:SI 5 "const_int_operand")))
        (mem:HI (plus:SI (match_dup 2)
                         (match_dup 5))))]
  "reload_completed
   && INTVAL (operands[5]) == INTVAL (operands[4]) + 2"
  [(set (mem:SI (plus:SI (plus:SI (match_dup 0) (match_dup 1))
                         (match_dup 4)))
        (mem:SI (plus:SI (match_dup 2) (match_dup 4))))])

;; Combine: move.b (a0),(a1) + move.b 1(a0),1(a1) -> move.w (a0),(a1)
;; Byte version - only safe on 68020+
(define_peephole2
  [(set (mem:QI (match_operand:SI 0 "register_operand"))
        (mem:QI (match_operand:SI 1 "register_operand")))
   (set (mem:QI (plus:SI (match_dup 0) (const_int 1)))
        (mem:QI (plus:SI (match_dup 1) (const_int 1))))]
  "reload_completed && !TUNE_68000_10"
  [(set (mem:HI (match_dup 0)) (mem:HI (match_dup 1)))])

;; Combine: move.b n(a0),n(a1) + move.b n+1(a0),n+1(a1) -> move.w n(a0),n(a1)
;; Byte version with offset - alignment check for 68000/68010
(define_peephole2
  [(set (mem:QI (plus:SI (match_operand:SI 0 "register_operand")
                         (match_operand:SI 2 "const_int_operand")))
  ...
  "reload_completed
   ...
   && (!TUNE_68000_10 || (INTVAL (operands[2]) & 1) == 0)"
  [(set (mem:HI (plus:SI (match_dup 0) (match_dup 2)))
