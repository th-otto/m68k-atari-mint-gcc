...
;; This recognizes:
;;   (set (zero_extract:SI dst 16 0) (zero_extend:SI hi_src))  ; swap-move-swap
;;   (set (strict_low_part (subreg:HI dst 2)) lo_src)          ; move.w
;; And converts to:
;;   move.w hi_src, dst
;;   swap dst
;;   move.w lo_src, dst
;; Pattern with explicit subreg in strict_low_part
(define_peephole2
...
;; GCC sometimes uses (reg:HI N) instead of (subreg:HI (reg:SI N) 2)
(define_peephole2
  [(set (zero_extract:SI (match_operand:SI 0 "register_operand" "")
                         (const_int 16)
                         (const_int 0))
...
;; Matches:
;;   (set tmp src)                      ; copy high value to temp
;;   (set tmp (ashift tmp 16))          ; shift to high position
;;   (set dst (and dst 0xffff))         ; mask (but followed by strict_low_part, so unnecessary)
;;   (set dst (ior dst tmp))            ; combine high bits
;;   (set (strict_low_part dst:HI) lo)  ; set low bits
;; Outputs the *pack_hi_hi_to_si pattern which generates:
;;   move.w hi,dst; swap dst; move.w lo,dst
;; Note: Uses const_int_operand predicates to avoid pointer comparison issues
(define_peephole2
  [(set (match_operand:SI 0 "register_operand" "")
